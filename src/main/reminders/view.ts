 ...
 * of one just created — and also of one whose fired step is still unanswered,
 * which `awaiting` is what tells the two apart.
 */
export const toReminderViews = (
  ...
  definitions.map((definition) => {
    const run = state.find((entry) => entry.definitionId === definition.id)
    return {
      ...definition,
      nextFireAt: run?.nextFireAt ?? null,
      awaiting: run !== undefined && run.nextFireAt === null,
    }
  })
